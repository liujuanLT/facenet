    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('squeezenet', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                ...
                net = slim.conv2d(images, 96+32, [7, 7], stride=2, scope='conv1')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool1')
                net = fire_module(net, 16+32, 64+64, scope='fire2')
                net = fire_module(net, 16+32, 64+64, scope='fire3')
                net = fire_module(net, 32+32, 128+64, scope='fire4')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool4')
                net = fire_module(net, 32+32, 128+64, scope='fire5')
                net = fire_module(net, 48+32, 192+64, scope='fire6')
                net = fire_module(net, 48+32, 192+64, scope='fire7')
                net = fire_module(net, 64+32, 256+64, scope='fire8')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool8')
                net = fire_module(net, 64+32, 256+64, scope='fire9')
                ...
                net = tf.math.reduce_mean(net, [1, 2], keep_dims=False, name='logits')
